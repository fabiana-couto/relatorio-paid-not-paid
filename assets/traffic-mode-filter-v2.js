(function(){
  ...
  const NOTPAID_MEDIA_VIEW=(DATA.views&&DATA.views.notpaid_media)?'notpaid_media':'notpaid';
  const NOTPAID_SOURCE={last:'notpaid_last',media:NOTPAID_MEDIA_VIEW};
  const SOURCE_TO_ATTR={last:'last',first:'first',media:'media',campaign:'campaign',notpaid_last:'last',notpaid:'media',notpaid_media:'media'};

  state.trafficMode=(String(state.source).startsWith('notpaid'))?'notpaid':'paid';
  state.attribution=SOURCE_TO_ATTR[state.source]||'last';

  function isNotPaid(){return state.trafficMode==='notpaid'||String(state.source).startsWith('notpaid')}
  ...
  function hideLegacyPageFilter(){
    const pageMenu=document.getElementById('pageMenu');
    const pagePop=pageMenu&&pageMenu.closest('.pop');
    if(pagePop) pagePop.style.display='none';
  }
  function removeInvestment(){
    if(!isNotPaid())return;
    document.querySelectorAll('.inv-panel,.investment-panel,[data-section="investment"],.kpi,section.panel,section.enh-panel').forEach(x=>{
      const txt=(x.textContent||'').trim();
      if(x.matches('.inv-panel,.investment-panel,[data-section="investment"]')||/^investimento\b/i.test(txt)) x.remove();
    });
  }
  function ensureFilterUI(){
    hideLegacyPageFilter();
    ...
    if(!mode){
      ...
      <div class="menu" id="trafficModeMenu" style="min-width:230px" onclick="event.stopPropagation()"><div class="menu-title">Tipo de tráfego</div><div class="menu-note">Investimento fica disponível somente em Paid.</div>
  ...
  function syncUI(){
    ...
    const note=document.getElementById('attrNote');if(note)note.textContent=isNotPaid()?'Not Paid: escolha Fonte recente ou Tipo de mídia / Canal.':'Paid: Fonte recente, Fonte original, Tipo de mídia / Canal ou Campanha de aquisição.';
    removeInvestment();
  }
  ...
  window.setSource=function(s){state.trafficMode=String(s).startsWith('notpaid')?'notpaid':'paid';state.attribution=SOURCE_TO_ATTR[s]||'last';apply()};
  ...
  frontPaceHTML=function(){
    const views=isNotPaid()?['notpaid_last',NOTPAID_MEDIA_VIEW]:['last','first','media','campaign'];
